...
**Run 218 (`34669793382`) is green at validator commit `e6451e64e13d79043d3d952098b4fb4ad5d75b44`, validating Nevada Pass 37 active and the validator extended through Pass 37.** Typecheck, curated-casino audit, inherited test-suite report, development build and Dinner icon verification all completed successfully. Run 218 supersedes Run 212 as the current validated runtime baseline. A green run never authorizes a merge to `main` by itself.
...
Casino runtime is **validated through Pass 37**. Pass 30 adds Oklahoma Duck Creek and Checotah; Pass 31 adds Artesian Casino; Pass 32 adds 15 Nevada NV-02 destinations from Laughlin and Reno-Sparks; Pass 33 adds the complete four-destination Texas set; Pass 34 adds seven QA-cleared rural Nevada destinations; Pass 35 adds ten QA-cleared outer-Clark/Mesquite/Primm Nevada destinations; Pass 36 adds six QA-cleared Boulder Strip/Henderson Nevada destinations; Pass 37 adds four QA-cleared North Las Vegas/northwest-valley destinations. `src/lib/nightlife/search.ts` imports and appends through Pass 37. `scripts/audit-casino-catalog.mjs` enumerates through Pass 37 and recognizes both inline jurisdiction literals and `JURISDICTION` helper constants. Run 218 validates Pass 37 green.
...
## Nevada — rolling, 92 active runtime destinations
...
`audit/nevada-north-las-vegas-coordinate-qa-pass-10-2026-09-12.json` clears **four North Las Vegas / northwest-valley destinations**: **Bighorn Casino, Cannery Casino & Hotel, Aliante Casino + Hotel + Spa and Santa Fe Station Hotel & Casino.** `src/lib/nightlife/casino-catalog-pass-37.ts` serializes those four and raises Nevada active curated runtime from **88 to 92**. Cannery remains distinct from the closed Eastside Cannery. Historical Aliante Station naming collapses to current Aliante rather than becoming a parallel destination. Run 218 validates Pass 37 green.

Follow-up reconciliation in `audit/nevada-north-las-vegas-followup-2026-09-12.json` clears **Jerry's Nugget Casino** for a future runtime increment at `36.1932,-115.13263`; it is not yet counted in the 92 active records. The former Poker Palace is now a retired name: the same property reopened in July 2026 as **Club Fortune North**, so future runtime work must use the successor identity and never serialize Poker Palace in parallel. **Silver Nugget Casino is excluded as closed** rather than retained as a current destination.

Nevada remains pending/rolling rather than statewide complete. Remaining work includes the rest of the Boulder/Henderson segment (Longhorn, Skyline, the Jokers Wild/Cadence Crossing succession, Club Fortune Henderson, Rainbow Club, Emerald Island, Pass Casino, Railroad Pass and Barley's), North Las Vegas follow-up including Club Fortune North, and remaining outer-Clark holds such as Arizona Charlie's Decatur and Rancho/Fremont Wildfire. Buffalo Bill's and Whiskey Pete's remain operation-state holds. Bonanza Casino Fallon remains held rather than promoted from weaker coordinate evidence; Red Drag Elko, The Nevada Casino & Bar Battle Mountain, Longstreet normalization and other rural completeness items remain staged until they independently clear the same gate.
...
`audit/texas-runtime-reconciliation-2026-09-12.json` completes stable-ID and duplicate/alias QA. Livingston and Leggett remain distinct current operating destinations. Repeated NIGC Naskila and Speaking Rock rows collapse by physical property. `src/lib/nightlife/casino-catalog-pass-33.ts` serializes all four Texas destinations, and the integration manifest marks Texas complete with expectedCount 4. Run 218 confirms the combined runtime remains green.
