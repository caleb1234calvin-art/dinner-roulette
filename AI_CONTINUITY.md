...
- During the first formal all-ID accounting, two CGCC IDs that existed in the 74-record inventory and first-pass research were found absent from the staged strict/special-case sets: **TRCS-000011 Wanaaha Casino** and **TRCS-000038 Win-River Casino**. They were recovered and reconciled in `audit/california-accounting-gap-recovery-2026-09-10.json` rather than silently disappearing.
- `audit/california-all-id-accounting-2026-09-10.json` now proves the complete inventory accounting: **74 unique inventory IDs = 69 property-reconciled IDs + 5 explicit special-case/runtime-decision IDs**, with zero overlap, zero missing IDs, and zero unexpected IDs. Set check result: **PASS**.
- The five explicit special-case IDs are Chicken Ranch (current resort primary; OG Casino documented separately/live discovery), Hidden Oaks (Class II scope hold), Casino Morongo (dedupe against verified Morongo Resort campus), Mechoopda (closed temporary property exclusion), and North Fork Mono (pre-opening exclusion until announced October 2026 opening).
- Cautions inside the 69 property-reconciled records remain explicit: Red Fox has an operating-status caution; The Heights/former Cher-Ae Heights has a regulator-locality caution; Acorn Ridge uses an authoritative casino project-site coordinate rather than a claimed front-door centroid.
- **California research and license-ID accounting are complete.** Do not start another broad research batch unless consolidation exposes contradictory evidence.
- California is now in **consolidation and runtime-set derivation**. Merge the staged evidence into one authoritative California reconciliation, derive the justified current physical destination set, and ensure special cases are represented according to the decisions above.
