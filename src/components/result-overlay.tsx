    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-bg"><div className="mx-auto flex min-h-dvh max-w-lg flex-col">
      {phase === "spin" ? <div className="flex flex-1 flex-col items-center justify-center px-6 text-center"><p className="text-kicker text-subtle">Choosing</p><div className="mt-6 w-full overflow-hidden rounded-xl bg-surface px-4 py-8 shadow-border"><p key={reel} className="reel-name font-display text-3xl text-fg">{reel}</p></div></div> : <>
        ...
        <div className="result-in px-5 pt-2 pb-10"><p className="text-kicker text-subtle">{resultKicker}</p><h2 className="font-display mt-2 text-4xl leading-tight text-fg">{restaurant.name}</h2><p className="mt-2 text-sm text-muted">{tagline}</p>
          ...
          <div className="mt-3 grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4"><div className="min-w-0"><p className="text-sm text-muted">{formatDistance(restaurant.distanceMiles)}{restaurant.hoursKnown ? ` · ${restaurant.isOpen ? restaurant.closesLabel ?? "Open" : "Closed"}` : ""}</p>{mode === "date-night" && !restaurant.hoursKnown ? <p className="mt-1 text-sm text-subtle">Hours unknown — check before going.</p> : null}{restaurant.closingSoon ? <p className="mt-1 text-sm text-danger">Closing soon — go now if you're in.</p> : null}<p className="mt-1 text-sm text-subtle">{restaurant.address}</p></div><RideshareQuickActions restaurant={restaurant} /></div>
          ...
          <div className="mt-6 space-y-2"><Button size="lg" className="w-full" asChild><a href={mapsUrl} target="_blank" rel="noreferrer"><MapPinned className="size-5" /><span className="tracking-kicker uppercase">Go here</span></a></Button>
            {mode === "date-night" && restaurant.website ? <Button size="lg" variant="secondary" className="w-full" asChild><a href={restaurant.website} target="_blank" rel="noreferrer"><ExternalLink className="size-4" /><span className="tracking-kicker uppercase">{websiteLabel}</span></a></Button> : null}
            ...
            {mode === "dinner" ? <div className="mt-4 rounded-xl bg-surface p-4 shadow-border"><div className="flex items-center gap-2"><Utensils className="size-4 text-accent" /><p className="text-xs tracking-[0.18em] text-subtle uppercase">Order instead</p></div><div className="mt-3 grid grid-cols-3 gap-2"><a className="rounded-lg bg-elevated px-2 py-3 text-center text-xs text-fg" href={doorDashSearchUrl(restaurant.name)} target="_blank" rel="noreferrer">DoorDash</a><a className="rounded-lg bg-elevated px-2 py-3 text-center text-xs text-fg" href={grubhubSearchUrl(restaurant)} target="_blank" rel="noreferrer">Grubhub</a><a className="rounded-lg bg-elevated px-2 py-3 text-center text-xs text-fg" href={uberEatsSearchUrl(restaurant.name)} target="_blank" rel="noreferrer">Uber Eats</a></div></div> : null}
            {mode === "nightlife" ? <div className="mt-4 flex items-center justify-center gap-2 text-xs text-subtle"><MoonStar className="size-4" />Make the night yours.</div> : null}
            {restaurant.phone ? <Button variant="ghost" className="w-full" asChild><a href={`tel:${restaurant.phone}`}><Phone className="size-4" />Call</a></Button> : null}
          </div>
        </div></>}
