...
import { formatOsmAddress } from "../location/model";
import { validCoordinates, validateNightlifeQuery, inactivePlace, heldCasino, casinoNamesMatch, nightlifeTags, nightlifeWebsite } from "./discovery-policy";
...
import { CASINO_CATALOG_PASS_52 } from "./casino-catalog-pass-52";
import { CASINO_CATALOG_PASS_53 } from "./casino-catalog-pass-53";
import { CASINO_CATALOG_PASS_54 } from "./casino-catalog-pass-54";
import { CASINO_CATALOG_PASS_55 } from "./casino-catalog-pass-55";
import { CASINO_CATALOG_PASS_56 } from "./casino-catalog-pass-56";
import { CASINO_CATALOG_PASS_57 } from "./casino-catalog-pass-57";
import { CASINO_CATALOG_PASS_58 } from "./casino-catalog-pass-58";
import { CASINO_CATALOG_PASS_59 } from "./casino-catalog-pass-59";
import { CASINO_CATALOG_PASS_60 } from "./casino-catalog-pass-60";
...
function mergeNightlife(live:NightlifePlace[],curated:NightlifePlace[]){const merged=[...curated];for(const place of live){const matchIndex=merged.findIndex((candidate)=>((candidate.venueTypes.includes("casino")&&place.venueTypes.includes("casino"))?casinoNamesMatch(candidate.name,place.name):nightlifeNamesMatch(candidate.name,place.name))&&haversineMiles(candidate.lat,candidate.lon,place.lat,place.lon)<0.35);if(matchIndex>=0){const saved=merged[matchIndex]!;merged[matchIndex]={...place,id:saved.id,name:saved.name,lat:saved.lat,lon:saved.lon,address:saved.address||place.address,cuisines:saved.cuisines,cuisineLabel:saved.cuisineLabel,priceLevel:saved.priceLevel??place.priceLevel,rating:saved.rating??place.rating,reviewCount:saved.reviewCount??place.reviewCount,openingHours:saved.openingHours||place.openingHours,phone:saved.phone??place.phone,website:saved.website??place.website,venueTypes:saved.venueTypes,energyLevel:saved.energyLevel,source:"merged"};continue}merged.push(place)}return merged}
