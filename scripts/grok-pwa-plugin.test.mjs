...
const EMPTY_ROOT = mkdtempSync(join(tmpdir(), "grok-pwa-fixture-"));

test("injects before </head>", () => {
  const out = injectGrokPwaHead("<html><head><title>x</title></head><body></body></html>", { cwd: EMPTY_ROOT });
...
test("does not duplicate grok:app_id", () => {
  const ctx = { appName: "Demo", projectId: "proj-123", cwd: EMPTY_ROOT };
...
test("does not duplicate x:creator tags", () => {
  const ctx = { appName: "Demo", projectId: "", creator: "@alice", creatorId: "42", cwd: EMPTY_ROOT };
  const once = injectGrokPwaHead("<html><head></head></html>", ctx);
  const twice = injectGrokPwaHead(once, ctx);
  assert.equal(once, twice);
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  const html = '<html><head><title>Hello World</title><meta property="og:title" content="Old"><meta name="twitter:card" content="summary"></head></html>';
...
test("a public card file wins over a baked site without card=custom", () => {
  ...
  const out = injectGrokPwaHead("<html><head></head></html>", { host: "wild-race.grok.me", cwd: root, site: {} });
  assert.match(out, /property="og:image" content="https:\/\/wild-race\.grok\.me\/og\.jpg"/);
  assert.doesNotMatch(out, /og\.grok\.me/);
});
...
test("public/og.png wins when jpg is absent", () => {
  ...
  const out = injectGrokPwaHead("<html><head></head></html>", { host: "wild-race.grok.me", cwd: root, site: { title: "Wild Race" } });
...
test("resolveOgCardAsset: disk file, then bake, then empty (placeholder)", () => {
  ...
  assert.equal(resolveOgCardAsset({ card: "custom", image: "/og.jpg" }, empty), "/og.jpg");
...
test("does not emit x:game:image without a public host or banner", () => {
  const noHost = injectGrokPwaHead("<html><head></head></html>", { site: { banner: "/x-banner.jpg" }, cwd: EMPTY_ROOT });
  assert.doesNotMatch(noHost, /x:game:image/);
  const noBanner = injectGrokPwaHead("<html><head></head></html>", { host: "wild-race.grok.me", site: { type: "x:game", card: "custom" }, cwd: EMPTY_ROOT });
  assert.doesNotMatch(noBanner, /x:game:image/);
});

test("site title Grok App is a real name, not a sentinel", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", { host: "wild-race.grok.me", site: { title: "Grok App" }, cwd: EMPTY_ROOT });
  assert.match(out, /property="og:title" content="Grok App"/);
});

test("published grok.me slug is still a title fallback", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", { host: "wild-race.grok.me", cwd: EMPTY_ROOT });
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  const themed = injectGrokPwaHead("<html><head></head></html>", { host: "wild-race.grok.me", site: { title: "Wild Race", color: "#FF4D2E" }, cwd: EMPTY_ROOT });
  ...
  const invalid = injectGrokPwaHead("<html><head></head></html>", { host: "wild-race.grok.me", site: { title: "Wild Race", color: "red" }, cwd: EMPTY_ROOT });
  assert.doesNotMatch(invalid, /color=/);
  const custom = injectGrokPwaHead("<html><head></head></html>", { host: "wild-race.grok.me", site: { title: "Wild Race", card: "custom", color: "FF4D2E" }, cwd: EMPTY_ROOT });
  assert.doesNotMatch(custom, /color=/);
});

test("document title entities are not double-escaped on og:title", () => {
  const out = injectGrokPwaHead("<html><head><title>Cats &amp; Dogs</title></head></html>", { cwd: EMPTY_ROOT });
...
test("site.json title wins over the host slug", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", { host: "wild-race.grok.me", site: { title: "Pixel Nova" }, cwd: EMPTY_ROOT });
  assert.match(out, /property="og:title" content="Pixel Nova"/);
});
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", { appName: "Wild Race", cwd: EMPTY_ROOT });
...
test("streaming injector passes post-head chunks through untouched", () => {
  const injector = createHeadInjector({ cwd: EMPTY_ROOT });
